...
### 타겟 로드맵 (차기 마이너 업데이트 1.7.0)

- **Elixir (Erlang BEAM) 지원 계획**: 기존 1.6 마이너 패치 라인업에 배정되었던 네이티브 Elixir(`<<tag::integer, rest::binary>>` 바이너리 패턴 매칭) 생성기 개발 건은, 1.6 버전의 거대한 코어 아키텍처 변화로 인해 한 단계 격상된 **v1.7.0** 정식 기능 릴리즈 마일스톤으로 승격 및 연기되었습니다.

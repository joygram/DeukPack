...
- **Docs / examples / kits**: paths updated for **`ts`** / **`js`**; **`project_i`** **`build-deukpack.js`** stages from **`buildDir/ts`**, with fallback to **`buildDir/typescript`**.
...
- **Docs**: DeukNavigation **game integration** (baking / runtime) expanded; `deukpack.app` submodule bumps; internal **project_i path standard** aligned (`clientDeukDefinePath`, UPM Plugins vs legacy “Assets/Core” wording removed).

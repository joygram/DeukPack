...
### 4. 고성능 인플레이스 재사용 (In-place Reuse)

극한의 효율성을 위해 설계되었습니다. `Unpack(cached, data)` 패턴을 통해 기존 인스턴스를 재사용함으로써 **메모리 할당 60~100% 감소**, **JS 파싱 속도 250% 향상**을 달성합니다.
...
> **반드시** 게임 시작 시 미리 할당(new)해 둔 객체를 이용해 데이터를 덮어쓰는 **`Hero.Unpack(cachedHero, bin);`** 방식을 사용하면 가비지(GC) 발생을 최소화할 수 있습니다. (※ 복합 구조체 리스트의 경우 원소 인스턴스 할당이 발생할 수 있으나, 컨테이너 및 최상위 객체는 재사용됩니다.)

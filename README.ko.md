...
- [성능 테스트 관련 기술 문서](https://github.com/joygram/DeukPack/blob/main/docs/DEUKPACK_GC_PERFORMANCE_MATRIX.ko.md)

| 언어 환경 | BMT 프레임워크 | 측정 성능 (속도) | 핵심 기술 및 비교 표 |
| :--- | :--- | :---: | :--- |
| **C# (.NET 10)** | `BenchmarkDotNet` | **1.31 μs** | Protobuf-net ➔ **2.2x** (In-place 재사용) |
| **Node.js (V8)** | `mitata` | **10.66 μs** | msgpack-lite ➔ **5x** |
| **C++ (Native)** | `Google Benchmark` | **0.35 μs** | 메모리 아레나 기반 평면 오프셋 뷰 |
| **Java (JVM)** | `JMH` | **1.35 μs** | G1GC 환경 힙 스파이크 억제 아키텍처 |
| **Python (C-API)** | `pytest-benchmark` | **~ 3,690 Kops** | 가상머신 간섭을 우회하는 Rust 가속 |
| **Elixir (BEAM)** | `Benchee` | **~ 31 ms** | **0 MB 가비지** (VM 네이티브 패턴 매칭 방어) |

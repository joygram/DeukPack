@echo off
REM DeukPack 클론 후 초기화 한 번에 실행 (Node 있으면 setup, 없으면 winget 또는 Docker로 setup)
...
where docker >con 2>&1
if %errorlevel% equ 0 (
  echo [bootstrap] Node not found. Using Docker to run setup...
  docker run --rm -v "%cd%:/app" -w /app node:18-alpine node scripts/setup.js %*
  exit /b %errorlevel%
)
...
if %errorlevel% neq 0 (
  echo [bootstrap] winget failed. Install Node from https://nodejs.org/ or install Docker.
  exit /b 1
)

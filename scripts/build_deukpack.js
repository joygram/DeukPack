...
function printCliUsage(opts = {}) {
    ...
    line('  deukpack path/to/root.deuk ./gen --csharp -r path/to/idls');
    line('');
    ...
    line('  --define-root <name>  IDL root folder (default: idls, legacy: _thrift)');
...
/**
 * Default job output base = defineRoot (e.g. idls) so emits land in idls/csharp, …/ts, …/js, …/cpp.
 * @param {object} job
 * @param {object} config
 * @returns {string}
 */
 ...
 * outputDir: optional; omitted = same as defineRoot (default idls). Per-language folders: csharp, cpp, ts, js (override outputLangSubdirs).
...
function parseOptions(args) {
    const options = {
        ...
        csharpNullable: true,            // --csharp-nullable  Enable #nullable enable (Default: true)
        csharpVersions: ['net8.0', 'netstandard2.0'], // --csharp-version <v1,v2>
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        switch (arg) {
            ...
            case '--wire-profile':
                ...
                break;
            case '--csharp-version':
                options.csharpVersions = args[++i].split(',').map(v => v.trim());
                break;
 ...
 */
function getDefaultCsprojContent(assemblyName = 'DeukDefine', targetFramework = 'netstandard2.0;net8.0') {
    const isModern = targetFramework.includes('net8.0') || targetFramework.includes('net6.0') || targetFramework.includes('net7.0');
    return `<?xml version="1.0" encoding="utf-8"?>
<!-- Generated by DeukPack. -->
<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    <RootNamespace>deukpack</RootNamespace>
  ...
  <!-- Fallback: DeukPack.Protocol at ../../../ relative to the subfolder (dist-test/csharp/{version}/). -->
  <ItemGroup Condition="'$(DeukPackProtocolProject)' == '' and Exists('$(MSBuildThisFileDirectory)..\\..\\..\\DeukPack.Protocol\\DeukPack.Protocol.csproj')">
    <ProjectReference Include="$(MSBuildThisFileDirectory)..\\..\\..\\DeukPack.Protocol\\DeukPack.Protocol.csproj" />
  </ItemGroup>
</Project>
`;
}
...
async function generateCSharp(engine, ast, outputDir, options = {}) {
    ...
    const versions = options.csharpVersions || ['net8.0'];
    const { CSharpGenerator } = require('../dist/codegen/CSharpGenerator');

    for (const version of versions) {
        console.log(`   🔸 Target Version: ${version}`);
        const csharpDir = path.join(outputDir, options.langOutputSubdirs.csharp, version);
        if (fsSync.existsSync(csharpDir)) {
            // Clean old files in the versioned dir to avoid duplicates if IDL changed
            const files = fsSync.readdirSync(csharpDir);
            for (const f of files) {
                if (f.endsWith('.cs') || f.endsWith('.csproj')) {
                    fsSync.unlinkSync(path.join(csharpDir, f));
                }
            }
        }
        ...
        // NRT is only supported in C# 8.0/netcoreapp3.0/netstandard2.1+. 
        // netstandard2.0 by default uses C# 7.3 which doesn't support NRT.
        const enableNullable = options.csharpNullable && (version.includes('net8.0') || version.includes('netstandard2.1'));

        const genOptions = {
            efSupport: options.ef === true,
            csharpNullable: enableNullable,
        ...
        for (const [filename, content] of Object.entries(csharpFiles)) {
            if (!filename || filename === 'nul.cs' || filename.startsWith('nul')) continue;
        ...
        if (options.emitCsproj !== false) {
            ...
            let csprojContent = getDefaultCsprojContent(projectName, version);
            csprojContent = csprojContent.replace('@@NULLABLE@@', enableNullable ? 'enable' : 'disable');
            await fs.writeFile(csprojPath, csprojContent, 'utf8');
            console.log(`   📄 Generated: ${version}/${csprojName}`);
        }
    }

    const generateTime = Date.now() - startTime;
    console.log(`✅ C# generated in ${generateTime}ms`);
}
...
async function generateJava(engine, ast, outputDir, options = {}) {
    console.log('🔧 Generating Java code...');
    const startTime = Date.now();

    const javaDir = path.join(outputDir, options.langOutputSubdirs.java);
    await fs.mkdir(javaDir, { recursive: true });

    const { JavaGenerator } = require('../dist/codegen/JavaGenerator');
    const generator = new JavaGenerator();
    const javaFiles = await generator.generate(ast, options);

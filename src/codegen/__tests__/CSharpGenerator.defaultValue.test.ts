...
describe('CSharpGenerator: 기본값 및 호환', () => {
  ...
  describe('필드 기본값 (Apache 호환)', () => {
    it('기본값이 있는 primitive 필드는 선언 시 해당 값으로 초기화된다 (Apache 호환: 프로퍼티)', () => {
      const authCs = generated['auth.cs'] ?? '';
    ...
    it('Write 시 기본값과 같으면 스킵하는 조건이 생성된다', () => {
      const authCs = generated['auth.cs'] ?? '';
      expect(authCs).toContain('skip when equals default');
      expect(authCs).toMatch(/if \(this\.Count\s*!=\s*0\)\s*\/\/ skip when equals default/);
      expect(authCs).toMatch(/if \(this\.Kind\s*!=\s*([\w.]+\.)?test_e\._NONE\)\s*\/\/ skip when equals default/);
      expect(authCs).toMatch(/if \(this\.Scale\s*!=\s*[\d.]+\).*skip when equals default/);
    });
  ...
  describe('디폴트 값 지정 시 struct 필드 (NRE 방지)', () => {
    it('디폴트가 지정된 struct 필드는 선언 시 초기화되어 null이 아니다', () => {
      const authCs = generated['auth.cs'] ?? '';
      expect(authCs).toMatch(/public gplat_define\.MsgInfo Msg_info\s*\{ get; set; \}\s*=/);
      expect(authCs).toMatch(/Msg_info\s*=\s*(new gplat_define\.MsgInfo\(\)|gplat_define\.MsgInfo\.CreateDefault\(\))/);
    });

    it('Read() 진입 시 모든 필드를 기본값으로 초기화하여 wire-absent 필드·null 방지', () => {
      const authCs = generated['auth.cs'] ?? '';
      expect(authCs).toContain('this.Msg_info = new gplat_define.MsgInfo()');
    ...
    it('디폴트 미지정 struct 필드에는 강제 CreateDefault() 초기화를 넣지 않는다', () => {
      const gplatCs = generated['gplat.cs'] ?? '';
  ...
  describe('DeukPack vs Apache 동작 정합성', () => {
    it('CreateDefault() 내부에서 디폴트가 지정된 struct 필드는 초기화된다', () => {
      const authCs = generated['auth.cs'] ?? '';
      expect(authCs).toMatch(/o\.Msg_info\s*=\s*(new gplat_define\.MsgInfo\(\)|gplat_define\.MsgInfo\.CreateDefault\(\))/);
    });

    it('특정 필드만 지정해도 미지정 하위 struct는 CreateDefault()로 할당된다 (Apache 방식)', () => {
      const authCs = generated['auth.cs'] ?? '';
      expect(authCs).toContain('MsgId = 1');
      expect(authCs).toMatch(/MsgResult\s*=\s*gplat_define\.MsgResult\.CreateDefault\(\)/);
    });

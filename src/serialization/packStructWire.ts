// @ts-nocheck
/** Deuk native pack (LE) — keep in sync with JsPackRuntime.js.tpl */
...
function _packWriteStructBody(a, schema, obj, schemas) {
  ...
  for (var i = 0; i < ids.length; i++) {
    ...
    if (v === undefined || v === null) {
      if (f.required) logMissingRequiredField(schema.name || 'Struct', f.name);
      continue;
    }
...
export function structToPackBinary(schema, obj, schemas) {
  var a = [];
...
export function structFromPackBinary(schema, buf, schemas) {
  ...
  for (var id in schema.fields) {
    ...
    var v = raw[f.name];
    if (v === undefined && f.defaultValue !== undefined && f.defaultValue !== null) v = f.defaultValue;
    if (v === undefined && f.required) logMissingRequiredField(schema.name || 'Struct', f.name);
